...
Data aktualizacji: 2026-09-10 (UDP + throttling + adaptacyjne audio).
...
| **Bandwidth shaping bitmap** (MS-RDPBCGR 2.2.14) | Token bucket na bajty bitmap wg zmierzonego bandwidth (70% pasma, floor 1 Mbps) | 100% — `DisplayBudget` w ironrdp-server (dispatch_display_update) | 80% — build + testy; do potwierdzenia E2E z mstsc przy wolnym łączu | ⚠️ do testu użytkownika |
...
| Output — kodek OPUS (WAVE_FORMAT_OPUS 0x704F) | MS-RDPSND — serwer oferuje OPUS 48 kHz stereo jako preferowany (~64 kbit/s), PCM jako fallback; kodowanie opus2 w `sound_real.rs` | 100% | 50% — wymaga testu E2E z mstsc (negocjacja OPUS potwierdzona u klientów opartych o ironrdp-rdpsnd-native) | ⚠️ do testu użytkownika |
| **Adaptacyjny pacing audio** (MS-RDPEA 2.2.3.8) | Held-time z Wave Confirm (EWMA) → drop przy backlogu >400 ms; dynamiczny bitrate OPUS 64↔32 kbit/s | 100% — `linrdp/src/sound_real.rs` (wave_confirm + sender) | 80% — desync po pauzie should być <0.5 s; do potwierdzenia | ⚠️ do testu użytkownika |
| Output — realny dźwięk systemowy (PipeWire/Pulse) | — | 100% — `linrdp/src/sound_real.rs` (monitor `linrdp_audio` w demonie **tej** sesji, wskazanym przez `session::gate::audio_target()`; sink i pipe-source tworzone w procesie przez libpulse, fragsize=40 ms — niska latencja, cap backlog ~1 s, realne timestampy Wave) | 60% — testy jednostkowe na ścieżkach per-sesja; odsłuch na 3389 i 3390 do potwierdzenia | ⚠️ do testu użytkownika |
...
| Mikrofon — dostarczenie audio do aplikacji Linuksa | — | 100% — `linrdp/src/mic.rs` (`MicFifo`) pisze do `module-pipe-source` **tej** sesji: `$XDG_RUNTIME_DIR/linrdp/mic.fifo`, otwierany leniwie i ponownie przy zmianie sesji, O_NONBLOCK (pakiet ginie, nigdy nie blokuje połączenia) | 0% — brak fizycznego mikrofonu w teście | ⚠️ do testu użytkownika |
...
| Funkcjonalność | Opis | Pokrycie kodu | Pokrycie testami | Status |
|---|---|---|---|---|
| Clipboard (MS-RDPECLIP) | 1.3.3 | 80% — `StubCliprdrBackend` (kanał negocjowany); brak realnego kopiowania między systemami | 0% — nieprzetestowane | ⚠️ stub |
...
| **Soft-Sync DVC → UDP** (MS-RDPEMT) | Po zestawieniu tunelu serwer wysyła DVC SoftSyncRequest (wszystkie otwarte DVC → reliable UDP); ruch DVC w obie strony przez tunel (`ServerEvent::SoftSyncToUdp`/`UdpTunnelData`) | 90% — ironrdp-dvc `request_reliable_udp`/`process_tunnel` + most w ironrdp-server/linrdp | 40% — brak testu z mstsc (klient musi odpowiedzieć SoftSyncResponse) | ⚠️ do testu użytkownika |
| Multi-transport (UDP) | MS-RDPBCGR 2.2.1.3.10 + MS-RDPEMT + MS-RDPEUDP2 | 90% — Initiate Multitransport Request po finalize (`with_multitransport` w ironrdp-server), UDP accept loop z TLS + walidacją cookie (`linrdp/src/udp.rs` + craty `ironrdp-rdpeudp`, `ironrdp-rdpemt`, `ironrdp-rdpeudp-tokio` z upstreamu); odpowiedź klienta (TRANSPORT_RSP) konsumowana na kanale IO | 50% — pętla lokalna E2E (example `udp_loopback`: handshake + TLS + tunnel + dane); test z mstsc (status "UDP") do potwierdzenia | ⚠️ do testu użytkownika |
