...
pub struct RdpeaiClient {
    /// Receives mic audio Data PDUs from the peer (MS-RDPEAI receive side).
    pub data_sink: Option<AudioPacketSink>,
...
impl RdpeaiClient {
    ...
    /// Register a sink receiving mic audio Data PDUs from the peer
    /// (LinRDP server role: mstsc streams client microphone audio here).
    pub fn set_data_sink(&mut self, sink: AudioPacketSink) {
    ...
    fn handle_version(&mut self, pdu: VersionPdu) -> PduResult<Vec<DvcMessage>> {
        ...
        // MS-RDPEAI 3.2.5.1.2: after the version exchange the initiator
        // (server) announces its supported capture formats.
        let formats = self.handler.supported_formats().to_vec();
        Ok(vec![
            Box::new(RdpeaiPdu::Version(VersionPdu::new(client_version))),
            Box::new(RdpeaiPdu::Formats(FormatsPdu::server(formats))),
        ])
    }
...
impl DvcProcessor for RdpeaiClient {
    ...
    fn process(&mut self, _channel_id: u32, payload: &[u8]) -> PduResult<Vec<DvcMessage>> {
        ...
        match pdu {
            ...
            RdpeaiPdu::DataIncoming => {
                debug!("AUDIO_INPUT DataIncoming signal from peer");
                Ok(Vec::new())
            }
            RdpeaiPdu::Data(data_pdu) => {
                // Mic audio from the client — deliver to the data sink.
                if let Some(sink) = self.data_sink.as_mut() {
                    sink(data_pdu.data.clone());
                }
                Ok(Vec::new())
            }
            RdpeaiPdu::OpenReply(_) => {
                warn!("Unexpected OpenReply on AUDIO_INPUT initiator");
                Ok(Vec::new())
            }
        }
    }
