...
use ironrdp_pdu::surface_commands::{
    ExtendedBitmapDataPdu, FrameAction, FrameMarkerPdu, SurfaceBitsPdu, SurfaceCommand,
};
...
pub(crate) struct UpdateEncoder {
    ...
    /// Whether bitmap updates travel as surface commands (`Set/Stream Surface
    /// Bits`) rather than legacy `Bitmap` updates.
    uses_surface_commands: bool,
    /// Client negotiated `SURFCMDS_FRAMEMARKER` (MS-RDPBCGR 3.2.5.1.5): each
    /// frame's surface commands are wrapped in a `Frame Marker` BEGIN/END pair
    /// so the client presents them as a single entity (MS-RDPBCGR 2.2.9.2.3).
    frame_marker: bool,
    /// Monotonic `frameId` for `Frame Marker` commands.
    frame_counter: u32,
}
...
impl UpdateEncoder {
    ...
    pub(crate) fn new(
        ...
        let uses_surface_commands = surface_flags.contains(CmdFlags::SET_SURFACE_BITS);
        let stream_surface_bits = surface_flags.contains(CmdFlags::STREAM_SURFACE_BITS);
        let bitmap_updater = if uses_surface_commands {
            match codecs {
                #[cfg(feature = "qoiz")]
                UpdateEncoderCodecs { qoiz: Some(id), .. } => {
                    BitmapUpdater::Qoiz(QoizHandler::new(id, stream_surface_bits)?)
                }
                #[cfg(feature = "qoi")]
                UpdateEncoderCodecs { qoi: Some(id), .. } => BitmapUpdater::Qoi(QoiHandler::new(id, stream_surface_bits)),
                UpdateEncoderCodecs {
                    remotefx: Some((algo, id)),
                    remotefx_quant,
                    ..
                } => BitmapUpdater::RemoteFx(RemoteFxHandler::new(
                    algo,
                    remotefx_quant,
                    id,
                    desktop_size,
                ...
                UpdateEncoderCodecs {
                    nscodec: Some((id, cll)),
                    ..
                } => BitmapUpdater::NsCodec(NsCodecHandler::new(id, cll, stream_surface_bits)),
                _ => BitmapUpdater::None(NoneHandler::new(stream_surface_bits)),
            }
        } else {
        ...
        Ok(Self {
            ...
            pointer_cache_size,
            large_pointer_flags,
            uses_surface_commands,
            frame_marker: surface_flags.contains(CmdFlags::FRAME_MARKER),
            frame_counter: 0,
        })
    }
...
enum State {
    ...
    BitmapDiffs {
        ...
        bitmap: BitmapUpdate,
        pos: usize,
        /// Frame Marker wrapping (MS-RDPBCGR 2.2.9.2.3): `Some(frameId)` when
        /// this update's surface commands are grouped into a frame, None when
        /// the client did not negotiate `SURFCMDS_FRAMEMARKER` or the update
        /// goes over the legacy Bitmap path.
        frame_id: Option<u32>,
        begin_sent: bool,
    },
    #[default]
            ...
            let res = match state {
                State::Start(update) => match update {
                    DisplayUpdate::Bitmap(bitmap) => {
                        ...
                        let frame_id = if !diffs.is_empty() && encoder.uses_surface_commands && encoder.frame_marker {
                            let id = encoder.frame_counter;
                            encoder.frame_counter = encoder.frame_counter.wrapping_add(1);
                            Some(id)
                        } else {
                            None
                        };
                ...
                State::BitmapDiffs {
                    ...
                    if let Some(frame_id) = frame_id.filter(|_| !begin_sent) {
                        // MS-RDPBCGR 2.2.9.2.3: open the frame before its
                        // first surface command so the client buffers them
                        // and presents the group atomically.
                        self.state = State::BitmapDiffs {
                            diffs,
                            bitmap,
                            pos,
                        ...
                        return Some(frame_marker(FrameAction::Begin, frame_id));
                    }
                    let Some(rect) = diffs.get(pos) else {
                        encoder.bitmap_update_framebuffer(bitmap, &diffs);
                        self.state = State::Ended;
                        // Close the frame after its last surface command.
                        return frame_id.map(|id| frame_marker(FrameAction::End, id));
                    };
...
impl BitmapUpdateHandler for NoneHandler {
    fn handle(&mut self, bitmap: &BitmapUpdate) -> ServerResult<UpdateFragmenter> {
        ...
        set_surface(bitmap, CodecId::None.as_u8(), &data, self.stream)
    }
}
...
impl RemoteFxHandler {
    fn new(algo: EntropyBits, quant: Quant, codec_id: u8, desktop_size: DesktopSize, stream: bool) -> Self {
        Self {
...
impl BitmapUpdateHandler for RemoteFxHandler {
    fn handle(&mut self, bitmap: &BitmapUpdate) -> ServerResult<UpdateFragmenter> {
        ...
        set_surface(bitmap, self.codec_id, &buffer[..len], self.stream)
    }
}
...
#[cfg(feature = "qoi")]
impl QoiHandler {
    fn new(codec_id: u8, stream: bool) -> Self {
        Self { codec_id, stream }
    }
}
...
impl BitmapUpdateHandler for QoiHandler {
    fn handle(&mut self, bitmap: &BitmapUpdate) -> ServerResult<UpdateFragmenter> {
        ...
        set_surface(bitmap, self.codec_id, &data, self.stream)
    }
}
...
#[cfg(feature = "qoiz")]
impl QoizHandler {
    fn new(codec_id: u8, stream: bool) -> ServerResult<Self> {
...
impl BitmapUpdateHandler for QoizHandler {
    fn handle(&mut self, bitmap: &BitmapUpdate) -> ServerResult<UpdateFragmenter> {
        ...
        set_surface(bitmap, self.codec_id, outb.as_slice(), self.stream)
    }
}
...
#[cfg(feature = "nscodec")]
impl NsCodecHandler {
    fn new(codec_id: u8, color_loss_level: u8, stream: bool) -> Self {
        Self {
            codec_id,
            color_loss_level,
            stream,
...
fn set_surface(bitmap: &BitmapUpdate, codec_id: u8, data: &[u8], stream: bool) -> ServerResult<UpdateFragmenter> {
    ...
    // MS-RDPBCGR 2.2.9.2.2: continuous streams of encoded bitmap data (video,
    // scrolling) use the Stream Surface Bits Command; Set Surface Bits is the
    // plain "blit this rectangle" variant for clients that did not negotiate
    // `SURFCMDS_STREAMSURFACEBITS`.
    let cmd = if stream {
        SurfaceCommand::StreamSurfaceBits(pdu)
    } else {
        SurfaceCommand::SetSurfaceBits(pdu)
    };
    Ok(UpdateFragmenter::new(
...
/// MS-RDPBCGR 2.2.9.2.3: a Frame Marker Command (`TS_FRAME_MARKER`) that lets
/// the client present a group of surface commands as a single frame.
fn frame_marker(action: FrameAction, frame_id: u32) -> ServerResult<UpdateFragmenter> {
    let cmd = SurfaceCommand::FrameMarker(FrameMarkerPdu {
        frame_action: action,
        frame_id: Some(frame_id),
    });
    Ok(UpdateFragmenter::new(
