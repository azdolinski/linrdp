...
pub struct RdpServerOptions {
    ...
    /// PDU (MS-RDPBCGR 2.2.15.1) on the MCS message channel in the Optional
    /// Multitransport Bootstrapping phase of the connection sequence (after
    /// licensing, 1.3.1.1), asking the client to establish a sideband RDP-UDP
...
impl PendingConnection {
    ...
    graphics_pipeline: bool,
        ...
        acceptor.set_multitransport_announce(multitransport.is_some());
        // MS-RDPBCGR 1.3.1.1: the Initiate Multitransport Request goes out in
        // the connection sequence itself, after licensing.
        acceptor.set_multitransport_request(multitransport.map(|request| ironrdp_acceptor::MultitransportRequest {
...
impl RdpServer {
        ...
        W: FramedWrite,
    {
        ...
        // A tunnel, and the request that offered it, last as long as the
        // connection (MS-RDPEMT 1.3.3); a Deactivation-Reactivation Sequence
        // keeps them.
        if !result.reactivation {
            self.udp_tunnel_tx = None;
        ...
        // What the client said on the message channel while the acceptor
        // waited for the Confirm Active and the finalization, such as the
        // Multitransport Response (MS-RDPBCGR 2.2.15.2).
        if let Some(message_channel_id) = result.message_channel_id {
            for user_data in &result.message_channel_pdus {
                self.handle_message_channel_data(SendDataRequest {
                    initiator_id: result.user_channel_id,
