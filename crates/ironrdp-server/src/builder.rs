...
pub struct BuilderDone {
    ...
    credential_resolver: Option<std::sync::Arc<dyn Fn(&str) -> std::io::Result<ironrdp_pdu::rdp::client_info::Credentials> + Send + Sync>>,
    enable_ainput: bool,
    dynamic_channel_attachers: Vec<Box<dyn FnMut(&mut ironrdp_dvc::DrdynvcServer) + Send>>,
    ...
    multitransport: Option<crate::server::MultiTransportRequest>,
}
        ...
        D: RdpServerDisplay + 'static,
    {
        RdpServerBuilder {
            state: BuilderDone {
                ...
                enable_ainput: false,
                dynamic_channel_attachers: Vec::new(),
                sound_factory: None,
...
impl RdpServerBuilder<BuilderDone> {
    ...
    /// Register a factory producing an extra dynamic virtual channel
    /// processor to attach to the DRDYNVC initiator on every connection.
    pub fn with_dynamic_channel_attacher(
        mut self,
        attacher: impl FnMut(&mut ironrdp_dvc::DrdynvcServer) + Send + 'static,
    ) -> Self {
        self.state.dynamic_channel_attachers.push(Box::new(attacher));
        self
    }
    ...
    /// Provide an MS-NLMP per-account secret resolver (SAM-style): maps the
    /// username the client presented to that account's password, used by
    /// CredSSP/NTLM per MS-RDPBCGR 5.4.2.
    /// Enable the MS-RDPEAI "Advanced Input" dynamic channel. Off by default:
    /// only enable when the client supports it and the handler is implemented.
    pub fn with_ainput(mut self, enable: bool) -> Self {
        self.state.enable_ainput = enable;
        self
    }

    pub fn with_credential_resolver(
        mut self,
        resolver: std::sync::Arc<dyn Fn(&str) -> std::io::Result<ironrdp_pdu::rdp::client_info::Credentials> + Send + Sync>,
    ) -> Self {
        self.state.credential_resolver = Some(resolver);
        self
    }
    ...
    pub fn with_multitransport(mut self, request: Option<crate::server::MultiTransportRequest>) -> Self {
        self.state.multitransport = request;
        self
    }
    ...
    pub fn build(self) -> RdpServer {
        let mut server = RdpServer::new(
            RdpServerOptions {
                ...
                multitransport: self.state.multitransport,
            },
            self.state.handler,
            self.state.display,
            self.state.static_channel_factories,
            self.state.dynamic_channel_attachers,
            ...
            self.state.credential_resolver,
            self.state.enable_ainput,
            #[cfg(feature = "egfx")]
