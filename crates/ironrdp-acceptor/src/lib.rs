...
use ironrdp_connector::sspi::{AuthIdentity, KerberosServerConfig};
use ironrdp_connector::{ConnectorError, ConnectorResult, ServerName};
    ...
    N: NetworkClient,
{
    ...
    where
        ...
        N: NetworkClient,
    {
        // MS-NLMP server model: the account secret is looked up by the
        // username the client presented in the NTLM NEGOTIATE message —
        // via the server-provided resolver (SAM-style store) when present,
        // otherwise against the single pre-loaded identity.
        let fallback = acceptor.creds.clone();
        let sam_resolver = acceptor.credential_resolver.clone();
        let resolver: credssp::CredentialResolver = std::sync::Arc::new(move |username| {
            if let Some(resolver) = &sam_resolver {
                let creds = resolver(username.account_name())?;
                return Ok(AuthIdentity {
                    username: username.clone(),
                    password: creds.password.into(),
                });
            }
            let Some(creds) = &fallback else {
                return Err(std::io::Error::other("no credentials while doing credssp"));
            };
            if username.account_name() != creds.username {
        ...
        let mut sequence =
            credssp::CredsspSequence::init(resolver, client_computer_name, public_key, kerberos_config)?;
