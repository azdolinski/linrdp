...
impl core::fmt::Debug for CredsspSequence {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("CredsspSequence").field("state", &self.state).finish()
    }
}

/// Resolves the account secret by username — the MS-NLMP server-side model
/// (a Windows LSA reads the account's secret from SAM at authentication time).
pub type CredentialResolver =
    Arc<dyn Fn(&Username) -> std::io::Result<AuthIdentity> + Send + Sync>;
...
impl CredentialsProxyImpl {
    fn new(resolver: CredentialResolver) -> Self {
        Self { resolver }
    }
}
...
impl CredentialsProxy for CredentialsProxyImpl {
    ...
    fn auth_data_by_user(&mut self, username: &Username) -> std::io::Result<Self::AuthenticationData> {
        // Look up the account secret exactly like a Windows LSA would.
        (self.resolver)(username)
    }

    fn auth_data(&mut self) -> Result<Vec<Self::AuthenticationData>, std::io::Error> {
        Err(std::io::Error::other(
            "enumerating all accounts is not supported; authentication is by username",
        ))
    }
}
...
impl CredsspSequence {
    ...
    pub fn init(
        ...
        let credentials = CredentialsProxyImpl::new(resolver);
